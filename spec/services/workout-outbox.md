...
When a flush cannot push because the device is effectively signed out — either `!authStore.isAuthenticated` at the top of `flushIfAuthenticated()`, or a `POST` returns 401 even after `withAuthorizedRequest` tried to refresh — the service must fail **recoverably**, and — *only when a session actually lapsed* — **loudly**:

**Lapsed session vs. never signed in (the loud/quiet gate).** The app is fully usable offline with no account; completions queuing locally on a device that has **never** signed in (or that deliberately signed out and kept training) is normal operation, not an error — exactly the reasoning that already gates the [auth-sync banner](#auth-sync-banner-ui) on `sessionExpired` (GH #143 follow-up). The loud path is reserved for genuine data-at-risk, i.e. `reason == "push_401"` (a live push was rejected server-side) **or** `authStore.sessionExpired == true` (the refresh chain died on a previously signed-in device — and note the dead-refresh-chain path inside `withAuthorizedRequest` sets `sessionExpired` before the flush reports, so the two conditions together cover every real auth breakage). Sentry LIFTMARK-IOS-P was a single never-signed-in TestFlight device emitting hundreds of these; that device now takes the quiet path.
...
2. **Reflect the unsynced state observably.** `lastError` is set to a user-facing "Sign in to sync" message and `pendingCount` continues to reflect the stranded rows so the [auth-sync banner](#auth-sync-banner-ui) and the Settings sync pill can render.

Loud path only (lapsed session / server 401):

3. **Log to the device log at error level.** `Logger.shared.error(.sync, "outbox flush blocked: authentication required", metadata: [...])` so the on-device debug log records the unsynced state (this is the authoritative on-device record per [logger.md](logger.md)).
4. **Capture to Sentry once per build per device** via `CrashReporter.shared.captureErrorOncePerBuild(key: "outbox-auth-failure", ...)` with `category: .sync`, `metadata["tag"] = "outbox_auth_failure"`, and `metadata["reason"]` (`"not_authenticated"` or `"push_401"`, so the two trigger paths are distinguishable in Sentry). The first flush cycle that hits the auth failure on a given build captures the full error; every later flush cycle on the same build emits a breadcrumb instead (still at most one per cycle, never per item). The `pendingCount` of stranded items is attached under the allowlisted `partialFailureCount` key in both cases. See [sentry.md](sentry.md) for the tag/allowlist and the shared once-per-build throttle.

Quiet path (never signed in): steps 1–2 plus an **info-level** device log (`"outbox flush skipped: not signed in"`) — no Sentry capture and no breadcrumb. Proven by `OutboxPusherServiceTests.testNeverSignedInFlushStaysQuiet`.

The loud path is the one normal-operation path (alongside the giveup-4xx case) that *does* emit a Sentry capture: a user who believed they were synced and completed a workout that silently never synced is a real, actionable failure, not a transient network blip. The server-401 half of the gate is proven by `OutboxPusherServiceTests.testPush401WhileSignedInCapturesLoudly`.

**Self-recovering, but kept error-level (GH #265); capture throttled once per build (Sentry LIFTMARK-IOS-P).** The strand normally self-heals: a 401 never bumps `next_attempt_after` (see [OutboxPusherService](#outboxpusherservice) — only transient 5xx/429/network failures schedule a backoff), so the row stays immediately eligible and drains on the next successful auth (login, app foreground, or launch). GH #265 confirmed there is **no data-loss bug** on builds ≥ 147 and chose to keep the event at error level: the signal is genuinely data-at-risk (the workout is unsynced until re-auth, and permanently stuck if the user never signs back in). GH #265's bet that per-cycle volume would stay low did **not** survive contact with reality: Sentry LIFTMARK-IOS-P showed a single persistently signed-out TestFlight device emitting 700+ identical events over a month — one per foreground flush. The condition is per-device chronic, not per-occurrence interesting, so the *capture* is throttled to once per build per device via the shared [`captureErrorOncePerBuild`](sentry.md#once-per-build-capture-throttling) machinery; repeats within a build drop to breadcrumbs. Each new build re-captures once, so the Sentry issue stays alive for as long as any device remains stranded, without the per-foreground spam. The recovery is proven end-to-end by `OutboxPusherServiceTests.testStrandedWorkoutDrainsAfterReLogin` — enqueue → lapsed-session strand (queue preserved, alert fired once with `partialFailureCount: 1`) → `login()` → the recovery flush pushes the stranded row and the queue empties. The throttle is proven by `OutboxPusherServiceTests.testRepeatedAuthFailureCapturesOncePerBuild` — a second lapsed-session flush emits a breadcrumb, not a second capture.
...
- `outbox flush blocked: authentication required` — `{pendingCount}` (logged via `Logger.shared.error(.sync, …)` every flush cycle; also a Sentry capture with `tag: "outbox_auth_failure"` on the first affected flush cycle per build, breadcrumbs thereafter)

No Sentry error captures for normal flows (network blips are not bugs; a never-signed-in device queuing completions locally is not a bug either). 4xx-other-than-429 is a real bug; log via Sentry. **Auth failure on a *previously signed-in* device with a non-empty queue is also a real, user-impacting bug** (a completed workout silently never synced — GH #143); it captures once per build per device, gated on lapsed-session-or-server-401 (see [Auth-failure handling during flush](#auth-failure-handling-during-flush)).
