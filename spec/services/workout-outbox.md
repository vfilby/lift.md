...
No Sentry error captures for normal flows (network blips are not bugs). 4xx-other-than-429 is a real bug; log via Sentry. **Auth failure with a non-empty queue is also a real, user-impacting bug** (a completed workout silently never synced — GH #143); it captures once per build per device (see [Auth-failure handling during flush](#auth-failure-handling-during-flush)).
